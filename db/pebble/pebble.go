...
func New(file string, cache int, handles int, namespace string, readonly bool) (*Database, error) {
	...
	logger := newColorLogger(w)
	logger = log.With(logger, "ts", log.DefaultTimestampUTC, "database", file)

	level.Info(logger).Log("msg", "Opening database", "cache", StorageSize(cache*1024*1024), "handles", handles)
...
// determines the number of records in the database
...
// func (d *PebbleDatabase) meter(refresh time.Duration) {
// 	var errc chan error
...
func newColorLogger(w io.Writer) log.Logger {
	return term.NewColorLogger(w, log.NewLogfmtLogger,
		func(keyvals ...interface{}) term.FgBgColor {
			if keyvals[0] == "a" {
				return term.FgBgColor{Fg: term.Cyan, Bg: term.Black}
			}
