...
func (bc *Blockchain) WriteRFQTxs(tx *types.Transaction) error {
	...
	case types.OpenRFQTxType:
		...
		case types.RFQStatusMatched:
			err = bc.matchedRFQSTable.Put(tx.ReferenceTxHash().Bytes(), encRFQ.Bytes())
		case types.RFQStatusSettled:
			err = bc.settledRFQSTable.Put(tx.ReferenceTxHash().Bytes(), encRFQ.Bytes())
		default:
