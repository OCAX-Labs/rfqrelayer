...
func (db *Database) NewBatch() rfqdb.Batch {
	return &batch{
		db: db,
	}
}
...
func (db *Database) NewBatchWithSize(size int) rfqdb.Batch {
	return &batch{
		db: db,
	}
}
...
func (db *Database) NewIterator(prefix []byte, start []byte) rfqdb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *Database) NewSnapshot() (rfqdb.Snapshot, error) {
